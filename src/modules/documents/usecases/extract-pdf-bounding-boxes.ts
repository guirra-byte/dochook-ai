...
class ExtractPdfBoundingBoxes {
  async execute(doc: FullfilledDocDownload) {
    ...
    const docBoundingBoxes = new Map<string, Boundarie[]>();
            ...
            if (textItem) {
              ...
              if (!docBoundingBoxes.has(docId)) {
                docBoundingBoxes.set(docId, [
                  { pageRef: page, boxes: [boundContent] },
                ]);
              } else {
                const prevData = docBoundingBoxes.get(docId);
                if (prevData) {
                  const pageBound = prevData.findIndex(
                    (bound) => bound.pageRef === page
                  );

                  if (pageBound !== -1) {
                    prevData[pageBound]?.boxes.push(boundContent);
                  } else {
                    prevData.push({
                      boxes: [boundContent],
                      pageRef: page,
...
export const extractPdfBoundingBoxes = new ExtractPdfBoundingBoxes();
